...
const SearchItem = ({ coin, onSelect, isActiveName }: SearchItemProps) => {
  ...
  // SearchCoin has data.price_change_percentage_24h as a number
  // TrendingCoin['item'] has data.price_change_percentage_24h as { usd: number }
  const isSearchCoinData = typeof data?.price_change_percentage_24h === 'number';
  const isTrendingCoinData = typeof (data?.price_change_percentage_24h as any)?.usd === 'number';

  const change = isSearchCoinData
    ? (data?.price_change_percentage_24h ?? 0)
    : isTrendingCoinData
      ? ((data?.price_change_percentage_24h as any)?.usd ?? 0)
      : 0;
